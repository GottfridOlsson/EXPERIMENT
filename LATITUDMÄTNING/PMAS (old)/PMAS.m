...
%     Updated: 2022-03-05, 20:59
%       About: PMAS version Gamma:
%              i) Plot protractor lines to lasercut
%                 a protractor
%%----------------------------------------------------%%

%% PLOT PROTRACTOR %% version Gamma %%
clc, clf


% --- Constants --- %
tickFontSize = 14; textFontSize = 17;

distanceBetweenDegreeLines = 1.2; %[mm]
numberOfLines = 180; %180 = 1 deg sep, 180*10 = 0.1 deg sep
R = distanceBetweenDegreeLines*numberOfLines/pi;  % [mm], radius of protractor
R = 
h_top = 20;                 % [mm], height on top of half-circle protractor
delta_th = 0.1;             % [deg], (delta) sub degrees on protractor
...
text_d = 20;                 % [mm], distance from l_th_10 to text
R_cut_d = 0.5;              % [mm], distance from R to laser cutting R_cut

l_th_10  = 28;              % [mm], length of 10 deg lines
l_th_5   = 10;             % [mm], length of 5  deg lines (sort of)
l_th_1   = 8;             % [mm], length of 1 degree lines (sort of)
l_th_sub = 4;             % [mm], length of sub  degree lines (sort of)

d_hole_screw = 0;           % [mm], diameter hole for screw
d_hole_plumb = 10;           % [mm], diameter hole for plumb bob

x_hole_screw_center = (R-2*6)/2; %from drawing of lighttube
...
% Text for myself
text(deg2rad(180+50), R/2.1, 'Gottfrid Olsson',...
    'HorizontalAlignment', 'center', 'Interpreter', 'latex',...
        'FontSize', textFontSize, 'Rotation', 0)
text(deg2rad(360-50), R/2.1, '2022-03-05',...
    'HorizontalAlignment', 'center', 'Interpreter', 'latex',...
        'FontSize', textFontSize, 'Rotation', 0)
text(deg2rad(180+90), R/4.7, 'PMAS version Gamma', ...
    'HorizontalAlignment', 'center', 'Interpreter', 'latex',...
    'FontSize', textFontSize, 'Rotation', 0)
% Plot Esthetics 
